...
from flext_core.domain.mixins import EntityMixin, MetadataMixin, StatusMixin
...
class LDAPConnection(DomainEntity, EntityMixin):
    ...
    host:
            str = Field(..., min_length=1, max_length=255, description="LDAP server host")
    port: int = Field(default=389, ge=1, le=65535, description="LDAP server port")

    # Authentication
    bind_dn:
        str | None = Field(None, description="Distinguished name for binding")
    bind_password:
            str | None = Field(None,
        description="Password for LDAP authentication",
    )
    use_ssl:
            bool = Field(default=False, description="Use SSL/LDAPS")
    use_tls: bool = Field(default=False, description="Use STARTTLS")

    # Search configuration
    base_dn:
        str = Field(..., min_length=1, description="Base DN for searches")
    search_scope:
            str = Field(default="subtree",)
        default="subtree",
...
class LDAPStream(DomainEntity, EntityMixin, MetadataMixin):
    ...
    connection_id:
            UUID = Field(..., description="Associated connection ID")
    stream_type: StreamType = Field(default=StreamType.CUSTOM,)
    ...
    replication_method: str = Field(default="FULL_TABLE",)
        default="FULL_TABLE",
...
class TapExecution(DomainEntity, EntityMixin, StatusMixin):
    ...
    tap_status: TapStatus = Field(default=TapStatus.IDLE,)
    ...
    streams_processed: int = Field(default=0,)
        default=0,
        ge=0,
    ...
    def is_completed(self) -> bool:
            return self.tap_status in {TapStatus.COMPLETED,
            TapStatus.FAILED,
            TapStatus.CANCELLED,
        }

    @property  def is_successful(self) -> bool:
...
class LDAPRecord(DomainEntity, EntityMixin):
    ...
    stream_id:
            UUID = Field(..., description="Associated stream ID")
    ...
    # Singer protocol
    singer_record: dict[str, Any] = Field(default_factory=dict,)
        default_factory=dict,
