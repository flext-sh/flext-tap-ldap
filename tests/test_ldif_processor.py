...
from flext_tap_ldap import FlextTapLdapLdifStreams, FlextTapLdapProcessor, t
...
def test_ldif_directory_processing_traverses_ldif_files(tmp_path: Path) -> None:
...
def test_transform_entry_applies_rules() -> None:
...
def test_directory_processing_traverses_ldap_dit_with_mock_connection(
    ...
    class _FakeConnection:
        def __init__(self, *args, **kwargs: t.Scalar) -> None:
            self.extend = Mock()
...
def test_transform_entry_applies_schema_mappings() -> None:
    transformer = Transformer(
        transformation_rules={
            "schema_mappings": {
                "uid": "employeeId",
