...
class TestLDIFProcessor:
    ...
    def test_filter_entries_by_objectclass(self, sample_ldif_content: str) -> None:
        ...
        if len(users) != EXPECTED_BULK_SIZE  # john and admin:
            raise AssertionError(f"Expected {2  # john and admin}, got {len(users)}")
    ...
    def test_filter_entries_by_dn_pattern(self, sample_ldif_content: str) -> None:
        ...
        if len(user_entries) != EXPECTED_BULK_SIZE  # john and admin:
            raise AssertionError(f"Expected {2  # john and admin}, got {len(user_entries)}")

        # Filter by specific pattern
        admin_entries = processor.filter_by_dn_pattern("cn=admin")
        if len(admin_entries) != 1:
            raise AssertionError(f"Expected {1}, got {len(admin_entries)}")
