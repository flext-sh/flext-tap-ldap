...
FLEXT Tap LDAP - Singer Tap for LDAP Directory Services

## Module Map

- Primary facades: `FlextTapLdapService`, `FlextTapLdapConstants`, `FlextTapLdapLdifStreams`, `FlextTapLdapStreams`, `FlextTapLdapTap`, `FlextTapLdapUtilities` (+7 more)
- Alias namespaces: `c`, `d`, `e`, `h`, `m`, `p`, `r`, `s`, `t`, `u`, `x`
- Public symbol exports: `14`
...
2. Confirm parent MRO chain via `pyproject.toml` `dependencies` filtered by `flext-*` (excluding `flext-tap-ldap` self).
...
- Public extensions exposed by this project: `FlextTapLdapService`, `FlextTapLdapConstants`, `FlextTapLdapLdifStreams`, `FlextTapLdapStreams`, `FlextTapLdapTap`, `FlextTapLdapUtilities` (+7 more).
